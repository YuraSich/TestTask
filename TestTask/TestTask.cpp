...
void SKeyboard(int key, int x, int y) {
  // Взависимости от первой нажатой кнопки шар примет то раправление и игра
  // "снимается" с паузы
  // TODO По хорошему стоит передать ответственность за выбор направления
...
void Restart() {
  ...
  for (int ax = 0 + 52; ax < WIN_HEI - 52; ax += 52) {
    for (int ay = 0 + 32; ay < WIN_HEI / 2; ay += 32) {
      ...
      // TODO По хорошему, шанс должен хоть как-то меняться (Например от уровня
      // сложности)
      if (rand() % 10 < 7) {
